 ...
 * ==============================================
 * One shared script loaded on every HTML page. Each init* function checks for DOM
 * elements before running, so pages only run what they need (no errors on “wrong” page).
 *
 * External libraries (cite in course / README):
 * - jQuery 4.x — https://jquery.com/
 *       Used only for: Read more/less (index.html), FAQ accordion (membership.html)
 ...
 *
 * Data flow (checkout demo):
 * checkout.html → Place order → JSON in sessionStorage key "monoMuseOrder"
 * order-confirmation.html → reads same key and renders summary (escHtml avoids XSS)
 */

/** Demo price per ticket; must match copy on checkout.html ($18 each). */
var TICKET_UNIT_USD = 18;

/**
 * Footer copyright: finds #copyYear and sets "© {currentYear} MonoMuse Museum..."
...
/**
 * Maps hour of day (0–23 from Date.getHours()) to a greeting fragment.
...
/**
 * Home page only: if #greeting exists, sets text from greetingByHour + museum welcome.
 * Safe no-op on other pages.
 */
...
/**
 * Derives the current HTML filename from location.pathname for nav highlighting.
...
/**
 * Highlights active nav item: compares each link’s pathname to currentPageFile().
 * Adds class "active" (styled in style.css). try/catch handles file:// or unusual hrefs.
 */
function ActiveNav() {
...
/**
 * Mobile nav: toggles class nav_open on <nav class="nav_bar"> and syncs aria-expanded on the hamburger.
...
/**
 * explore.html: creates Leaflet map if global L exists and #map is in the DOM.
 * Coordinates approximate Oakland / museum location; marker popup labels the museum.
 */
 ...
 * sessionStorage is per-tab and cleared when the tab closes (fine for a class demo).
 */
...
/** Immediately invoked: attaches hamburger click once; no need to wait for DOMContentLoaded (script at end of body). */
 ...
 * - #readMore / #readLess / #longIntro: progressive disclosure on home
 * - .faq-trigger: membership page accordion (slideToggle + aria-expanded)
 */
